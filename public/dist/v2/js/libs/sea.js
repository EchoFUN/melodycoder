...
(function(global, undefined) {"use strict"
	...
	var forEach = emptyArr.forEach ? function(arr, fn) {
		arr.forEach(fn)
	} : function(arr, fn) {
	...
	var log = seajs.log = function() {
		...
		fn = fn.apply ? fn : Function.prototype.bind.call(fn, console)
	...
	var loaderUri = getScriptAbsoluteSrc(loaderScript) || pageUri// When `sea.js` is inline, loaderUri is pageUri
	...
	function preload(callback) {
		...
		len ? globalModule.load(preloadModules.splice(0, len), callback) : callback()
	}
	...
	function checkConfigConflict(prev, curr, k, key) {
		if (prev !== curr) {
			log("The config of " + key + '["' + k + '"] is changed from "' + prev + '" to "' + curr + '"', "warn")
		}
	}
	...
	function makeBaseAbsolute() {
		...
		if (!isAbsolute(base)) {
			configData.base = id2Uri((isRoot(base) ? "" : "./") + base + (base.charAt(base.length - 1) === "/" ? "" : "/"))
		}
	}
